...
"""
From https://raw.githubusercontent.com/aurora95/Keras-FCN/master/utils/SegDataGenerator.py
Will be part of keras-contrib in future (see PR https://github.com/keras-team/keras-contrib/pull/202 )
"""
            ...
            if self.target_size:
                if self.crop_mode != 'none':
                    ...
                    if self.data_format == 'channels_first':
                        x = np.lib.pad(x, ((0, 0), (pad_h // 2, pad_h - pad_h // 2), (pad_w // 2, pad_w - pad_w // 2)),
...
def get_steps_per_epoch(length, batch_size, gpu_n=1):
    # from Keras documentation: Total number of steps (batches of samples) to yield from generator before declaring
    # one epoch finished and starting the next epoch. It should typically be equal to the number of unique samples of
    #  your dataset divided by the batch size.

    return int(np.ceil(length / float(batch_size * gpu_n)))
...
if __name__ == '__main__':
    ...
    train_file_path = os.path.join(data_path, 'gta/train/images.txt')
    flen = get_file_len(train_file_path)
    # steps_per_epoch = get_steps_per_epoch(flen, 2)
    # print(flen, steps_per_epoch)
    #
    what = train_datagen.flow_from_directory(
        file_path=train_file_path,
        data_dir=os.path.join(data_path, 'gta/train/images/'),
        data_suffix='.png',
        label_dir=os.path.join(data_path, 'gta/train/labels/'),
        label_suffix='.png',
